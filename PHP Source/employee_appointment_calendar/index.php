...
.modal-footer	 {
    ...
    width: 30%;
		...
		<script>
			...
			function addAppt(comment,start,did,pid)
			{
				...
				//console.log("webservice/webservice.php?method=addApptPid&comment=" + comment + "&start="+start + "&did="+did + "&pid="+pid);
				
				//REQUEST TO WEBSERVICE
				xhttp.open("GET", "../webservice/webservice.php?method=addApptPid&comment=" + comment + "&start="+start + "&did="+did + "&pid="+pid, false);
				xhttp.send();
				//location.reload();
			}
	...
	function getData(start, end)
	{
		var noAccess = "Appointment";
			...
			console.log("end: " + end);
			xhttp.open("GET", "../webservice/webservice.php?method=getAppt&start=" + start + "&end=" + end+"&did=" + doctorDisplayed, false);
			xhttp.send();
	...
	function getPatientName(pid)
	{
		...
		xhttp.open("GET", "../webservice/webservice.php?method=getPatientName&pid=" + pid, false);
		xhttp.send();
		console.log(xhttp.responseText);
		var response = JSON.parse(xhttp.responseText);
		...
		else //If everything was okay
		{
			...
			{
				var userObject = response.data[index]; //Get the user at a specific index
				var add = {first : userObject.first, last : userObject.last};
				test.push(add);
	...
	function refresh(start,end) { // document ready
		...
		daysOff = getOffData(start,end);
		
		$('#calendar').fullCalendar('removeEvents');
		$('#calendar').fullCalendar('addEventSource', events);
		$('#calendar').fullCalendar('addEventSource', daysOff);
		...
		$(function() { // document ready
			...
			console.log(date);
			  ...
			  $('#calendar').fullCalendar({
				  ...
				  maxTime: "18:00:00",
				  height: 520,
				  alldayslot: false,customButtons: {
					  Doctor1: {
				            text: 'Doctor 1',
				            click: function(){
				            	doctorDisplayed = 1;
			    ...
			    eventClick: function(calEvent, jsEvent, view) {
				    ...
				    var name = getPatientName(calEvent.id);
				    var fullName = "Patient Name: " + name[0].first + " " + name[0].last;
				    var reason = "Reason: " + calEvent.title;
				    var startTime = "Start Time: " + calEvent.start._i;
				    var PID = "Patient ID: " + calEvent.id;

			        
			        $('#editAppointment #reason').text(reason);
			        $('#editAppointment #patientID').text(PID);
			        $('#editAppointment #startTime').text(startTime);
			        $('#editAppointment #Name').text(fullName);
			  ...
			  function doEdit(){
				  ...
				  var startTime = ($('#editAppointment #startTime').text());
				  var pid = ($('#editAppointment #patientID').text());
				  startTime = startTime.split(" ");
				  var passedTime = startTime[2] + " " + startTime[3];
				  pid = pid.split(" ");
				  var passedPid = pid[2];
				  console.log(passedTime);

				  window.location = "../appointment/appointment_edit.php?Date=" + passedTime + "&Patient_id="+ passedPid + "&Doctor_id="+doctorDisplayed;
			  }
			  ...
			  function doSubmit(){
				    ...
				    addAppt($('#comment').val(),date,doctorDisplayed,$('#pid').val());
...
<div id="createEventModal" class="modal hide" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
    ...
    <form id="createAppointmentForm" class="form-horizontal">
        ...
        <div class = "control-group">
        	...
        	<div class="controls">
        		<input type = "text" name="pid" id="pid" tyle="margin: 0 auto;" data-provide="typeahead" data-items="4" data-source="[&quot;Value 1&quot;,&quot;Value 2&quot;,&quot;Value 3&quot;]">
...
<div id="editAppointment" class="modal hide" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
        <h3 id="myModalLabel1">Appointment Information</h3>
    </div>
    <div class="modal-body">
    <form id="editAppointmentForm" class="form-horizontal">
        <div class="control-group">
            <div class="controls">
                 <div class="controls controls-row" id="reason" style="margin-top:5px;">
                  <input type="hidden" id="apptStartTime"/>
                  <input type="hidden" id="apptEndTime"/>
                  <input type="hidden" id="apptAllDay" />
            </div>
        ...
        <div class = "control-group">
        	 ...
        	 <div class="controls controls-row" id="patientID" style="margin-top:5px;"> 
        ...
        <div class = "control-group">
        	 <div class="controls controls-row" id="Name" style="margin-top:5px;">
        	
        	</div>
        </div>
        <div class="control-group">
            <div class="controls controls-row" id="startTime" style="margin-top:5px;">
            </div>
        </div>
    </form>
    </div>
    <div class="modal-footer">
        <button class="btn" data-dismiss="modal" aria-hidden="true">Cancel</button>
        <button type="edit" class="btn btn-primary" id="editButton">Edit</button>
    </div>
</div>
